...
import { validatePhotoboardSettings } from "@/app/api/_lib/settingsMain";
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const nextValue = validatePhotoboardSettings(body?.value);
		if (!nextValue) {
			return jsonError(400, "Invalid data structure in request body");
		}

		await mainDocRef.set({ photoboard: nextValue }, { merge: true });
		const updatedMainDoc = await mainDocRef.get();
		return NextResponse.json({ main: updatedMainDoc.data() });
	} catch (error) {
		console.error("Error updating photoboard settings:", error);
		return jsonError(500, "Failed to update photoboard settings");
	}
}
