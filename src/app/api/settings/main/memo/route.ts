...
import { validateMemoSettings } from "@/app/api/_lib/settingsMain";
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const nextValue = validateMemoSettings(body?.value);
		if (!nextValue) {
			return jsonError(400, "Invalid data structure in request body");
		}

		await mainDocRef.set({ memo: nextValue }, { merge: true });
		const updatedMainDoc = await mainDocRef.get();
		return NextResponse.json({ main: updatedMainDoc.data() });
	} catch (error) {
		console.error("Error updating memo settings:", error);
		return jsonError(500, "Failed to update memo settings");
	}
}
